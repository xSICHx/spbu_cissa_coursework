...
### Сравнение разложений всего временного ряда через метод Фурье и CiSSA

```{r}
reconstruct_fft <- function(x, y) {
...
data$cos_err[1] <- mse(y1, r_cissa_grouped$sesonal_sin) |> 
  formatC(format = "e", digits = 1)
data$sin_err[1] <- mse(y2, r_cissa_grouped$sesonal_cos) |> 
  formatC(format = "e", digits = 1)
...
data$sin_err[2] <- mse(y2, r_fft_grouped$sesonal_cos) |>
  formatC(format = "e", digits = 1)
# data$noise_err[2] <- mse(y3, r_fft_grouped$residuals) |>
#   formatC(format = "e", digits = 1)
# data$signal[2] <- mse(y1+y2,
#                       r_fft_grouped$sesonal_sin + r_fft_grouped$sesoonal_cos) |>
#   formatC(format = "e", digits = 1)
...
data$sin_err[1] <- mse(y2, r_cissa_grouped$sesonal_cos) |> 
  formatC(format = "e", digits = 1)
# data$noise_err[1] <- mse(y2, r_cissa_grouped$residuals) |> 
#   formatC(format = "e", digits = 1)
# data$signal[1] <- mse(y1+y2,
#                        r_cissa_grouped$sesonal_cos+r_cissa_grouped$sesonal_sin) |> 
#   formatC(format = "e", digits = 1)
...
r_cissa_extended <- circulant_SSA(y, L, extend_flag = TRUE)
r_cissa_grouped_extended <- grouping_cissa(r_cissa_extended,
                    groups = list(
                      e = c(0, 1/12-eps-eps),
                      sesonal_sin = c(1/12-eps, 1/12+eps),
                      sesonal_cos = c(1/8-eps, 1/8+eps)
...
data$cos_err[1] <- mse(y1, r_fft_grouped$sesonal_sin) |>
  formatC(format = "e", digits = 1)
data$sin_err[1] <- mse(y2, r_fft_grouped$sesonal_cos) |>
  formatC(format = "e", digits = 1)
data$exp_err[1] <- mse(y3, r_fft_grouped$e) |>
  formatC(format = "e", digits = 1)
# data$signal[2] <- mse(y1+y2,
#                       r_fft_grouped$sesonal_sin + r_fft_grouped$sesoonal_cos) |>
#   formatC(format = "e", digits = 1)
...
data$cos_err[2] <- mse(y1, r_cissa_grouped$sesonal_sin) |> 
  formatC(format = "e", digits = 1)
data$sin_err[2] <- mse(y2, r_cissa_grouped$sesonal_cos) |> 
  formatC(format = "e", digits = 1)
data$exp_err[2] <- mse(y3, r_cissa_grouped$e) |> 
  formatC(format = "e", digits = 1)


data$cos_err[3] <- mse(y1, r_cissa_grouped_extended$sesonal_sin) |> 
  formatC(format = "e", digits = 1)
data$sin_err[3] <- mse(y2, r_cissa_grouped_extended$sesonal_cos) |> 
  formatC(format = "e", digits = 1)
data$exp_err[3] <- mse(y3, r_cissa_grouped_extended$e) |> 
  formatC(format = "e", digits = 1)
