...
# Шаг 2: Создание примера данных
data <- data.frame(
  Метод = c("SSA", "SSA EOSSA","Fourier", "CiSSA", "CiSSA extended"),
  sin_err = c (1, 20, 20, 1, 1),
  cos_err = c(1, 1, 1, 1, 1)
)
...
data <- list(
  ...
  sin_err = list(list(), list(), list(), list()),
  cos_err = list(list(), list(), list(), list())
)
...
for (i in 1:100){
  ...
  data$cos_err[[1]][[i]] <- mse(y1, r_ssa_e$sesonal_sin)
  data$sin_err[[1]][[i]] <- mse(y2, r_ssa_e$sesonal_cos)
  
  
  data$cos_err[[2]][[i]] <- mse(y1, r_fft_grouped$sesonal_sin)
  data$sin_err[[2]][[i]] <- mse(y2, r_fft_grouped$sesonal_cos)
  
  
  data$cos_err[[3]][[i]] <- mse(y1, r_cissa_grouped$sesonal_sin)
  data$sin_err[[3]][[i]] <- mse(y2, r_cissa_grouped$sesonal_cos)
  
  
  data$cos_err[[4]][[i]] <- mse(y1, r_cissa_grouped_ext$sesonal_sin)
  data$sin_err[[4]][[i]] <- mse(y2, r_cissa_grouped_ext$sesonal_cos)
}
library(xtable)
...
data$sin_err[2] <- mse(y2, r_fft_grouped$sesonal_cos) |>
  formatC(format = "e", digits = 1)
data$e_err[2] <- mse(y3, r_fft_grouped$e) |>
  formatC(format = "e", digits = 1)
...
data$sin_err[3] <- mse(y2, r_cissa_grouped$sesonal_cos) |> 
  formatC(format = "e", digits = 1)
data$e_err[3] <- mse(y3, r_cissa_grouped$e) |>
  formatC(format = "e", digits = 1)
...
data$sin_err[4] <- mse(y2, r_cissa_grouped_ext$sesonal_cos) |> 
  formatC(format = "e", digits = 1)
data$e_err[4] <- mse(y3, r_cissa_grouped_ext$e) |>
  formatC(format = "e", digits = 1)
...
data <- list(
  ...
  sin_err = list(list(), list(), list(), list()),
  cos_err = list(list(), list(), list(), list()),
  exp_err = list(list(), list(), list(), list())
)
...
for (i in 1:100){
  ...
  data$cos_err[[1]][[i]] <- mse(y1, r_ssa_e$sesonal_sin)
  data$sin_err[[1]][[i]] <- mse(y2, r_ssa_e$sesonal_cos)
  data$exp_err[[1]][[i]] <- mse(y3, r_ssa_e$e)
  
  
  data$cos_err[[2]][[i]] <- mse(y1, r_fft_grouped$sesonal_sin)
  data$sin_err[[2]][[i]] <- mse(y2, r_fft_grouped$sesonal_cos)
  data$exp_err[[2]][[i]] <- mse(y3, r_fft_grouped$e)
  
  
  data$cos_err[[3]][[i]] <- mse(y1, r_cissa_grouped$sesonal_sin)
  data$sin_err[[3]][[i]] <- mse(y2, r_cissa_grouped$sesonal_cos)
  data$exp_err[[3]][[i]] <- mse(y3, r_cissa_grouped$e)
  
  
  data$cos_err[[4]][[i]] <- mse(y1, r_cissa_grouped_ext$sesonal_sin)
  data$sin_err[[4]][[i]] <- mse(y2, r_cissa_grouped_ext$sesonal_cos)
  data$exp_err[[4]][[i]] <- mse(y3, r_cissa_grouped_ext$e)
}
library(xtable)
